...
function Lynx(host, port, options) {
  ...
  // The size of the MTU is configurable, but defaults to 1500. 1500 is
  // the typical size of an ethernet datagram.
...
*/
  ...
  var buffer = new Buffer(this.MTU);
  var ptr = 0;
  var stat;

  while(stat = stats_to_send.shift()) {

    if (stat.length > buffer.length) {
      this.on_error(
        { message : "Single stat is larger than configured datagram MTU; skipping"
        , f       : 'send'
        , args    : arguments
        });
    } else {

      if (ptr + stat.length > buffer.length) {
        this.writeBuffer(buffer.slice(0, ptr - 1), socket);
        ptr = 0;
      }
      buffer.write(stat, ptr);
      ptr += stat.length;
    }
  }
