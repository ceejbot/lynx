...
function Lynx(host, port, options) {
  ...
  // Handle prefix
  //
  this.scope = options.scope || options.prefix || '';
  ...
  // The size of the MTU is configurable, but defaults to 1476 - 28.
  // 1476 is the typical MTU of a tunnel, minus 28 bytes for headers.
  // This is a conservative choice.
  this.MTU = options.MTU || 1448;
  this.MTU = Math.max(68, parseInt(this.MTU, 10));
  ...
  //
  this.on_error = typeof options.on_error === 'function'
               ? options.on_error
...
Lynx.prototype.send = function send(stats, sample_rate) {
  ...
  var stats_to_send = all_stats.map(function construct_stat(stat) {
  ...
  while(stat = stats_to_send.shift()) {

    if (stat.length > this.MTU) {
      this.on_error(
        { message : 'Stat is larger than configured datagram MTU; skipping'
        , f       : 'send'
        , args    : arguments
        });
    } else {

      if (buffer.length && buffer.length + stat.length >= this.MTU) {
        this.writeBuffer(new Buffer(buffer, 'utf8'), socket);
        buffer = '';
      }
      if (buffer.length) {
        buffer += '\n';
...
Lynx.prototype.getSocket = function getSocket() {
  var self = this
  //
...
Lynx.prototype.writeBuffer = function writeBuffer(buffer, socket) {
  //
